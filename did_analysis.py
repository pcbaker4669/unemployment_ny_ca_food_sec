...
def run_did():
    # Paths
    data_file = "data/unemployment_data.csv"
    results_dir = "results"
    ...
    long_df = df.melt(id_vars="date", var_name="state", value_name="unemp_rate")
    ...
    # DiD regression
    model = smf.ols("unemp_rate ~ treated + post_policy + treated:post_policy", data=long_df).fit()
    ...
    pivot_df = long_df.pivot(index="date", columns="state", values="unemp_rate")
    plt.figure(figsize=(10, 6))
    pivot_df.plot(ax=plt.gca())
    plt.axvline(pd.to_datetime("2024-04-01"), color='red', linestyle='--', label='Policy Date')
    plt.title("Unemployment Rates: CA vs TX (2022–2025)")
    ...
    long_df_ny = df_ny.melt(id_vars="date", var_name="state", value_name="unemp_rate")
    long_df_ny["treated"] = (long_df_ny["state"] == "New York").astype(int)
    long_df_ny["post_policy"] = (long_df_ny["date"] >= "2025-01-01").astype(int)

    model_ny = smf.ols("unemp_rate ~ treated + post_policy + treated:post_policy", data=long_df_ny).fit()
    ...
    pivot_df_ny = long_df_ny.pivot(index="date", columns="state", values="unemp_rate")
    plt.figure(figsize=(10, 6))
    pivot_df_ny.plot(ax=plt.gca())
    plt.axvline(pd.to_datetime("2025-01-01"), color='red', linestyle='--', label='Policy Date')
    plt.title("Unemployment Rates: NY vs TX (2022–2025)")
    plt.ylabel("Unemployment Rate (%)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
